...
class MongoDBMessagesProjection(object):

    def __init__(self, host="localhost", port=27017, db_name="messages_projection"):
        self.client = MongoClient(host=host, port=port)
        self.messages_per_user = self.client[db_name].messages_per_user
        self.num_unread_msgs_per_user = self.client[db_name].num_unread_messages_per_user
    ...
    def mark_msg_as_read(self, user_id, message_id, new_seq_num):
        ...
        self._update_num_unread_msgs(user_id)
        self._update_seq_num(new_seq_num)

    def add_msg_for_user(self, user_id, message_id, new_seq_num):
        self.messages_per_user.insert(
            {"user_id": user_id, "message_id": message_id, "read": False})
        self._update_num_unread_msgs(user_id)
        self._update_seq_num(new_seq_num)
    ...
    def _update_num_unread_msgs(self, user_id):
        count = self.messages_per_user.find(
            {"user_id": user_id, "read": False}).count()
        self.num_unread_msgs_per_user.update_one(
